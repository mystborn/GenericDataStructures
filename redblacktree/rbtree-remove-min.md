...
Removes the node with the minimum key from the tree.

## Syntax

```c
bool rbtree_remove_min(RBTree* tree, value_type* out_value);
```
...
while(is_tree_remove_min(tree, &value)) {
    printf("%s\n", value);
}
