...
Gets the [intersection](https://en.wikipedia.org/wiki/Set_%28mathematics%29#Intersections) of two sets.

## Syntax

```c
bool set_intersect(Set* left, Set* right, Set* result);
```
...
This function gets the intersions of the left and right sets and stores the values in the result set. The result set __cannot__ be the same as the left or right sets.
...
str_set_intersect(left, right, result);
