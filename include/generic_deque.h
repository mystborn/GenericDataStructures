...
#define DEQUE_DEFINE_H(type_name, function_prefix, value_type) \
 ...
 \
    ...
    static inline void function_prefix ## _free_resources(type_name* deque) { gds_free(deque->buffer); } \
    static inline void function_prefix ## _free(type_name* deque) { gds_free(deque->buffer); gds_free(deque); } \
...
#define DEQUE_DEFINE_C(type_name, function_prefix, value_type) \
 ...
 \
    static bool function_prefix ## _resize(type_name* deque) { \
        ...
        deque->capacity *= 2; \
        value_type* buffer = gds_malloc(deque->capacity * sizeof(value_type)); \
        if(!buffer) { \
            deque->capacity /= 2; \
            return false; \
        } \
